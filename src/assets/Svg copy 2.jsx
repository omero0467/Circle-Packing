  ...
  <svg width={width} viewBox={`-${width / 2} -${width /2} ${width} ${width}`} style={{backgroundColor:" #ffffffbd"}} height={width}>
    {arr.map((element,i)=>{
      return <g textAnchor='middle'>
        <circle onMouseOver={handleHover} r={element.length*1.25} cx={-width/2 +25} transform={`translate(${(i%19)*50},${Math.floor(i/30)*70})`} opacity={0.5} ></circle>
        <text x={-width/2 +25} transform={`translate(${(i%19)*50},${Math.floor(i/30)*70})`}>{element}</text>
        </g>
    })}
  </svg>
